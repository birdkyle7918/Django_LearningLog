...
def topics(request):
    ...
    # 把数据库中该用户所有的topics全都拿出来作为上下文
    context = {'topics': topics}
    return render(request, 'learning_logs/topics.html', context)


@login_required
def topic(requrst, topic_id):
    # 显示单个主题和其所有条目
    topic = Topic.objects.get(id=topic_id)
    # 确认请求的topic属于当前用户
    if topic.owner != requrst.user:
        raise Http404
    entries = topic.entry_set.order_by('-date_added')    # -号指的是降序排序
    context = {'topic': topic, 'entries': entries}
    return render(requrst, 'learning_logs/topic.html', context)
...
def delete_topic(request, topic_id):
    ...
    context = {'topic_to_be_deleted_id': topic_to_be_deleted_id}
